...
const gwAddress						= '127.0.0.1';
const gwPort						= 9999;

const openhabUseCollector 			= 0;
const openhabCollectorItem 			= 'ArduinoRestInput';
const openhabUrl					= 'http://localhost:8080';
...
e.pullWord = function(arr, pos) {
	return arr[pos] + 256 * arr[pos + 1];
}
 
e.pushWord = function(arr, val) {
	arr.push(val & 0x00FF);
	arr.push((val  >> 8) & 0x00FF);
}

e.pushDWord = function(arr, val) {
...
e.loadFirmware = function(fwtype, fwversion, sketch, db) {
	var filename = path.basename(sketch);
        ...
        e.requestify.post('https://codebender.cc/utilities/compile/', req).then(function(res) {
...
e.encode = function(destination, sensor, command, acknowledge, type, payload) {
...
e.saveProtocol = function(sender, payload, db) {
...
e.saveSensor = function(sender, sensor, type, db) {
	db.collection('node', function(err, c) {
		c.update({
			'id': sender
		}, {
...
e.saveValue = function(sender, sensor, type, payload, db) {
...
e.saveBatteryLevel = function(sender, payload, db) {
...
e.saveSketchName = function(sender, payload, db) {
	db.collection('node', function(err, c) {
		c.update({
			'id': sender
		}, {
...
e.saveSketchVersion = function(sender, payload, db) {
	db.collection('node', function(err, c) {
		c.update({
			'id': sender
		}, {
...
e.sendTime = function(destination, sensor, gw) {
	var payload = new Date().getTime();
	var command = C_INTERNAL;
	var acknowledge = 0; // no ack
	var type = I_TIME;
	var td = e.encode(destination, sensor, command, acknowledge, type, payload);
	console.log('-> ' + td.toString());
	gw.write(td);
}

e.sendNextAvailableSensorId = function(db, gw) {
	db.collection('node', function(err, c) {
		c.find({
			$query: { },
			$orderby: {
...
e.sendConfig = function(destination, gw) {
	var payload = "M";
	var sensor = NODE_SENSOR_ID;
	var command = C_INTERNAL;
	var acknowledge = 0; // no ack
...
e.sendFirmwareConfigResponse = function(destination, fwtype, fwversion, db, gw) {
	...
	if (fwtype == 0xFFFF) {
		...
		// take predefined type (ideally selected in UI prior to dbConn of new sensor)
	...
	db.collection('firmware', function(err, c) {
		c.findOne({
			...
			if (err)
				...
				var payload = [];
				e.pushWord(payload, result.type);
				e.pushWord(payload, result.version);
				e.pushWord(payload, result.blocks);
				e.pushWord(payload, result.crc);
				var sensor = NODE_SENSOR_ID;
...
e.sendFirmwareResponse = function(destination, fwtype, fwversion, fwblock, db, gw) {
	db.collection('firmware', function(err, c) {
		c.findOne({
			'type': fwtype,
			'version': fwversion
...
e.saveRebootRequest = function(destination, db) {
	db.collection('node', function(err, c) {
		c.update({
			'id': destination
		}, {
...
e.checkRebootRequest = function(destination, db, gw) {
	db.collection('node', function(err, c) {
		c.find({
			'id': destination
		}, function(err, item) {
			if (err)
				...
				e.sendRebootMessage(destination, gw);
		});
	});
}

e.sendRebootMessage=function(destination, gw) {
	var sensor = NODE_SENSOR_ID;
        var command = C_INTERNAL;
        var acknowledge = 0; // no ack
        var type = I_REBOOT;
        var payload = "";
        var td = e.encode(destination, sensor, command, acknowledge, type, payload);
        if(debug) console.log('-> ' + td.toString());
        gw.write(td);
}
...
e.relayToOpenhab = function(sender, sensor, command, ack, type, payload, raw) {
	var itm;
	if(openhabUseCollector == 1){
		itm = openhabCollectorItem;
	}else{
		itm = 'n' + sender + "s" + sensor;
	}
	var targetUrl = openhabUrl + '/rest/items/' + itm;
	e.request({
		method: 'POST',
		uri: targetUrl,
		gzip: false,
	  ...
	  if(response.statusCode == 404){
		console.log("OpenHAB doesn't know about item: " + itm);
	  }
    });
}
e.send = function(sender, sensor, command, ack, type, payload) {
	var td = e.encode(destination, sensor, command, ack, type, payload);
	console.log('[e]-> ' + td.toString());
	gw.write(td);
}
e.rfReceived = function(data, db, gw) {
	if ((data != null) && (data != "")) {
		...
		switch (command) {
			case C_PRESENTATION:
				if (sensor == NODE_SENSOR_ID)
					e.saveProtocol(sender, payload, db);
				e.saveSensor(sender, sensor, type, db);
				break;
			case C_SET:
				e.saveValue(sender, sensor, type, payload, db);
				break;
			case C_REQ:
				break;
			case C_INTERNAL:
				...
				case I_BATTERY_LEVEL:
					e.saveBatteryLevel(sender, payload, db);
					break;
				case I_TIME:
					e.sendTime(sender, sensor, gw);
					break;
				case I_VERSION:
					break;
				case I_ID_REQUEST:
					e.sendNextAvailableSensorId(db, gw);
					break;
				case I_ID_RESPONSE:
					break;
				...
				case I_CHILDREN:
					break;
				case I_SKETCH_NAME:
					e.saveSketchName(sender, payload, db);
					break;
				case I_SKETCH_VERSION:
					e.saveSketchVersion(sender, payload, db);
					break;
				case I_REBOOT:
					break;
				}
			...
			case C_STREAM:
				switch (type) {
						case ST_FIRMWARE_CONFIG_REQUEST:
								...
								e.sendFirmwareConfigResponse(sender, fwtype, fwversion, db, gw);
								break;
						...
						case ST_FIRMWARE_REQUEST:
								...
								e.sendFirmwareResponse(sender, fwtype, fwversion, fwblock, db, gw);
								break;
		...
		e.relayToOpenhab(sender, sensor, command, ack, type, payload,data);
		e.checkRebootRequest(sender, db, gw);
	}
}

e._dbc = require('mongodb').MongoClient;
e.start = function(){
	e._dbc.connect('mongodb://' + dbAddress + ':' + dbPort + '/' + dbName, function(err, db) {
	if(err) {
	...
	for (var i = 0; i < fwSketches.length; i++) loadFirmware(i, 1, fwSketches[i], db);
	...
	if (gwType == 'Ethernet') {
		...
		gw.on('connect', function() {
			...
			e.appendData(rd.toString(), db, gw);
		}).on('end', function() {
			console.log('disconnected from gateway');
		}).on('error', function() {
			console.log('dbConn error - trying to reconnect');
			gw.connect(gwPort, gwAddress);
			gw.setEncoding('ascii');
		});
		...
		gw.on('open', function() {
			...
			console.log('dbConn error - trying to reconnect' + e);
			gw.open();
		});
	} else {
